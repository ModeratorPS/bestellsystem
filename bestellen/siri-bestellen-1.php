...
$query1 = "SELECT * FROM `Artikelliste` WHERE `artikel` = \"$art\" and `lager` != '0'";
$result1 = mysqli_query($link, $query1); 
$num_rows1 = mysqli_num_rows( $result1 ); 
if ($num_rows1 == 1) {
  while($zeile = mysqli_fetch_array( $result1, MYSQLI_ASSOC))
  {
    echo 'Meinst du '.$zeile['artikel'].' welcher '.$zeile['preis'].' Euro kostet?';
  }
} else if ($num_rows1 > 1) {
  while($zeile = mysqli_fetch_array( $result1, MYSQLI_ASSOC))
  {
    echo $zeile['artikel'].' welcher '.$zeile['preis'].' Euro kostet.<br>';
  }
} else {
