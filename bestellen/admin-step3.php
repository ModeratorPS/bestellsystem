...
$sql1 = "SELECT * FROM `checkout` WHERE `name` = \"$input1\"";
$db_erg1 = mysqli_query( $link, $sql1 );

$num_rows = mysqli_num_rows( $db_erg1 ); 

if ( $num_rows == 0 ) {
	$query2 = "INSERT INTO `checkout` (`name`, `Euro`) VALUES ('$input1', '$input2');"; 
	...
	while ($zeile = mysqli_fetch_array( $db_erg1, MYSQLI_ASSOC)) {
		...
		$query3 = "UPDATE `checkout` SET `Euro` = '\"$ergebnis\"' WHERE `name` = \"$input1\";"; 
...
$query = "UPDATE `bestellungen` SET `Status` = '2' WHERE `Name` = \"$input1\" AND `Status` = '1'"; 
...
?>
<h1>Status ändern auf "Fertig"</h1>
<form action="/bestellen/admin-step4.php" method="post" name="form">
...
$query = "SELECT `Name` FROM `bestellungen` WHERE `Name` = \"$input1\"";
$result = mysqli_query($link, $query); 
