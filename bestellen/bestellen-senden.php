...
while ($zeile = mysqli_fetch_array( $db_erg1, MYSQLI_ASSOC))
{
  if ($zeile['Status'] == "Geschlossen") {
    ...
    $trk2 = str_replace("tracking.php", "bestellt-error.php?code=474", $LINK_TRACKEN);
    echo '<meta http-equiv="refresh" content = "1;url='.$trk2.'">';
  }
  if ($zeile['Status'] == "Geöffnet") {
    $sql_12 = "SELECT * FROM `rabattcodes` WHERE `code` = \"$input5_code\"";
    $sql_result_12 = mysqli_query( $link, $sql_12 );
    $num_row_12 = mysqli_num_rows($sql_result_12);
    if ($input5_code != "") {
      while ($zeile12 = mysqli_fetch_array( $sql_result_12, MYSQLI_ASSOC)) {
        if ($zeile12['code'] == $input5_code) {
          $code = True;
          $new = $zeile12['verwendungen'] - 1;
          $query13 = "UPDATE `rabattcodes` SET `verwendungen`=\"$new\" WHERE `code` = \"$input5_code\""; 
	        $result13 = mysqli_query($link, $query13);
        }
        if ($zeile12['verwendungen'] <= 0) {
          echo '<h4 align="center" class="u-text-palette-3-base"><strong>Rabattcode prüfen . . .</strong></h4>';
          $trk2 = str_replace("tracking.php", "Bestellen.php?code=".$input5_code, $LINK_TRACKEN);
          echo '<meta http-equiv="refresh" content = "1;url='.$trk2.'">';
          exit;
        }
      }
    ...
    $sql2 = "SELECT * FROM `Artikelliste` WHERE `artikel` = \"$input3\"";
    $db_erg2 = mysqli_query( $link, $sql2 );
    while ($zeile2 = mysqli_fetch_array( $db_erg2, MYSQLI_ASSOC)) {
      if ($zeile2['TG'] == "Ja") {
        echo '<div align="center">
          <br><br><form action="bestellen-tgsenden.php?name='. $input1 .'&mail='. $input2 .'&bestellung='. $input3 .'&zusatz='. $input4 .'&id='. $input5 .'&code='. $input5_code .'" method="post" name="form">
            <strong>Bitte wähle die Tassengröße aus: </strong><br><select id="input1" name="input1">
              <option>Klein</option>
              <option selected>Mittel</option>
              <option>Groß</option>
            </select><br><br>
            <input type="submit" name="submit" id="submit" value="Senden" class="menu_button">
          </form><br><br>
        </div>';
        exit;
      }
    ...
    $query5 = "SELECT * FROM `Artikelliste` WHERE `artikel` = \"$input3\"";
    $result5 = mysqli_query($link, $query5); 
    while($zeile = mysqli_fetch_array( $result5, MYSQLI_ASSOC)) {
      if ($zeile['bedingung'] == "Mittel") {
        echo '<div align="center">
          <br><br><form action="bestellen-alter-senden.php?name='. $input1 .'&mail='. $input2 .'&bestellung='. $input3 .'&zusatz='. $input4 .'&id='. $input5 .'&code='. $input5_code .'" method="post" name="form">
            <strong>Bitte gebe dein Geburtsjahr ein: </strong><br><input type="number" style="width: 60px;" value="'.date('Y').'" id="input2" name="input2"><br><br>
            <input type="submit" name="submit" id="submit" value="Senden" class="menu_button">
          </form><br><br>
        </div>';
        exit;
      }
      if ($zeile['bedingung'] == "Stark") {
        if ($_SESSION["loggedin"] !== True) {
          $trk2 = str_replace("tracking.php", "bestellt-error.php?code=808", $LINK_TRACKEN);
          echo '<h4 align="center" class="u-text-palette-3-base"><strong>Bestellung senden . . .</strong></h4>';
          echo '<meta http-equiv="refresh" content = "1;url='.$trk2.'">';
          exit;
        }
      ...
      if ($zeile['bedingung'] == "Ultra") {
        echo '<div align="center">
          <br><br><form action="bestellen-passwort-senden.php?name='. $input1 .'&mail='. $input2 .'&bestellung='. $input3 .'&zusatz='. $input4 .'&id='. $input5 .'&code='. $input5_code .'" method="post" name="form">
            <strong>Dieser Artikel ist Passwortgeschützt.<br>Bitte gebe das Passwort ein: </strong><br><input type="password" id="input2" name="input2"><br><br>
            <input type="submit" name="submit" id="submit" value="Senden" class="menu_button">
          </form><br><br>
        </div>';
        exit;
    ...
    $query = "INSERT INTO `bestellungen` (`Name`, `E-Mail`, `Bestellung`, `Zusatz`, `ID`, `Status`, `code`) VALUES ('$input1', '$input2', '$input3', '$input4', '$input5', '0', '$input5_code');"; 
