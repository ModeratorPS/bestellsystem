...
$query = "UPDATE `bestellungen` SET `Status` = '1' WHERE `Name` = \"$input1\" AND `Status` = '0'"; 
$result = mysqli_query($link, $query); 
if( $result )
 {
 	echo 'Keine Fehler beim Bearbeiten!';
 }
 else
 {
...
$sql1 = "SELECT * FROM `bestellungen` WHERE `Name` = \"$input1\" AND `Status` = '1'";
...
?>
<h1>Status ändern auf "Auf dem Weg"</h1>
<form action="/bestellen/admin-step3.php" method="post" name="form">
...
$sql3 = "SELECT * FROM `bestellungen` WHERE `Name` = \"$input1\"";
...
$query = "SELECT * FROM `Artikelliste` WHERE `Artikel` = \"$order\""; 
$result = mysqli_query($link, $query); 
