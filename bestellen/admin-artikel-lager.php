...
<html style="font-size: 16px;" lang="de"><head>
    ...
    <title>Artikel lager bearbeiten</title>
...
<form action="/bestellen/admin-artikel-lager-senden.php" method="post" name="form">
<select id="input1" name="input1">
<?php
require_once "config.php";

$query = "SELECT `artikel` FROM `Artikelliste`";
$result = mysqli_query($link, $query); 
while($zeile = mysqli_fetch_array( $result, MYSQLI_ASSOC)) {
    echo '<option value="'.$zeile['artikel'].'">'.$zeile['artikel'].'</option>';  
}          
?> 
</select><br>
Anzahl (-1 für Unendlich): <input type="number" placeholder="100" id="input2" name="input2" required=""><br>
