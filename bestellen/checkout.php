...
while ($zeile = mysqli_fetch_array( $db_erg1, MYSQLI_ASSOC))
{
    ...
    echo '<td><form action="/bestellen/checkout-senden.php?name='. $zeile['name'] .'" method="post" name="form">';
    echo '<input type="submit" name="submit" id="submit" class="menu_button" value="Bezahlt">';
    echo '</form></td>';
    echo '<td><form action="/bestellen/ckeckout-level.php?name='. $zeile['name'] .'" method="post" name="form">';
    echo '<input type="submit" name="submit" id="submit" class="menu_button" value="Level Checkout">';
    echo '</form></td>';
    echo "</tr>";
}
echo "</table>";
...
?>
<br><a href="/bestellen/admin.php">Zurück</a>
