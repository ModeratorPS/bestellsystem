    ...
    <section class="u-clearfix u-section-1" id="carousel_9c88">
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          <?php
          $id = $_GET['id'];
          $sql_edit_order = "UPDATE `bestellungen` SET `Status` = '1' WHERE `ID` = \"$id\" AND `Status` = '0'"; 
          $sql_edit_order_result = mysqli_query($link, $sql_edit_order);
          $sql_select_order = "SELECT * FROM `bestellungen` WHERE `ID` = \"$id\" AND `Status` = '1'";
          ...
          foreach ($sql_select_order_result as $row) {
            ...
            echo "<td><strong>". $row['Bestellung'] . "</strong></td>";
            echo "<td>". $row['Name'] . "</td>";
          ...
          echo "</table>";
          ?>
          <h4>Status ändern auf "Auf dem Weg"</h4>
          <form action="edit2.php" method="post" name="form">
          <select id="input1" name="input1">
          <?php
          foreach ($sql_select_order_result as $row) {
          echo 'ID: <option value="'.$row['ID'].'">'.$row['ID'].'</option>'; 
          echo 'Preis: <input type="text" id="input2" name="input2" placeholder="Preis Produkte" require="" value="'.$row['total'].'">';
          }          
          ?> 
          </select>
