...
if ($_POST["1"] != "") {
    foreach ($sql_achievement_result as $row) {
        $id = $row["id"];
        $value = $_POST[$id];
        $sql_update = "UPDATE `achievement` SET `count`='$value' WHERE `id`='$id'";
        $sql_update_result = mysqli_query($link, $sql_update);
    }
    header('location: achievement.php?info=success');
} else if ($_GET['info'] != "success") {
  $type = "apple_on";
  $info = 'Einstellungen erfolgreich gespeichert!';
}
?>
<!DOCTYPE html>
<html style="font-size: 16px;" lang="de"><head>
    ...
    <title>🧪 - Achievements verwalten</title>
    ...
    <section class="u-clearfix u-section-1" id="carousel_9c88">
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          ...
          <form action="achievement.php" method="post" name="form">
          <h4>Achievements:</h4>
          <table>
            <?php
            foreach ($sql_achievement_result as $row) {
                echo "<fieldset>";
                echo '<div class="number_count"><input type="text" id="'.$row["id"].'" name="'.$row["id"].'" required="" value="'.$row["count"].'">x '.$row["type"]." für ".$row["achievement"]."</div>";
                echo "</fieldset>";
            }
            ?>
          </table><br>
          <input type="submit" class="u-border-2 u-border-black u-btn u-button-style u-hover-black u-none u-text-hover-white u-btn-1" name="submit" id="submit" value="SPEICHERN">
          </form>
        </div>
      </div>
    </section>
