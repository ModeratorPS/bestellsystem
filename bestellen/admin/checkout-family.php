...
if ($_GET['payedname'] != "") {
  $name = $_GET['payedname'];
  $sql_delete = "DELETE FROM `checkout` WHERE '$name' IN `name`";
    ...
    <section class="u-clearfix u-section-1" id="carousel_9c88">
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          <?php
          $name_un = str_replace("%20", " ", $_GET['name']);
          $name = explode(" - ", $name_un);
          $name_lastElement = end($name);
          $sql_checkouts = "SELECT * FROM `checkout` WHERE '$name_lastElement' IN `name`"; 
          $sql_checkouts_result = mysqli_query($link, $query);
          $txt = "<strong>Rechnung ".$lastElement."</strong>";
          $total = 0;
          foreach ($sql_checkouts_result as $row) {
            $txt = $txt."<br>".$zeile['name']." - ".str_replace('"', "", $zeile['Euro'])."€";
            $total = $total + intval(str_replace('"', "", $zeile['Euro']));
          }
          $txt = $txt."<br><br>Total: ".strval($total)."€";
          echo $txt;
          echo "<br><br><a href='checkout-family.php?payedname=". $name_lastElement ."' class='u-border-2 u-border-black u-btn u-button-style u-hover-black u-none u-text-hover-white u-btn-1'>BEZAHLT</a>"
          ?>
        </div>
      </div>
    </section>
