      ...
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          <div class="apple_settings">
            <p style="padding: 10px 10px 10px 10px"><strong>Neuen Tisch hinzufügen:</strong>
              <form action="tischnummer.php?action=new" method="post" name="form" style="padding: 10px 10px 10px 10px">
                <input style="padding: 5px 5px 5px 5px" type="number" name="name" id="name" required="" placeholder="Nummer des Tisches"><br><br>
                <input style="padding: 5px 5px 5px 5px" type="submit" id="submit" class="settings">
              </form>
            </p>
          </div><br>
          <?php
            $sql_tische = "SELECT * FROM `tische`";
            $sql_tische_result = mysqli_query($link, $sql_tische);
            foreach ($sql_tische_result as $row) {
              if ($row['status'] == 'on') {
                $text = "✅ - Tisch deaktivieren";
                echo '<div class="apple_'.$row['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>Tisch '.$row['nummer'].'</strong><p style="padding: 0px 0px 0px 10px"><a href="tischnummer.php?name='.$row['nummer'].'&status='.$row['status'].'&action=ch" class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br>';
              } else if ($row['status'] == 'off') {
                $text = "❌ - Tisch aktivieren";
                echo '<div class="apple_'.$row['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>Tisch '.$row['nummer'].'</strong><p style="padding: 0px 0px 0px 10px"><a href="tischnummer.php?name='.$row['nummer'].'&status='.$row['status'].'&action=ch" class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p><p style="padding: 0px 0px 0px 10px"><a href="tischnummer.php?name='.$row['nummer'].'&action=del" class="settings" style="color: black; padding: 5px 5px 5px 5px">Löschen</a></p></p></p></div><br>';
              }
            }
          ?>
        </div>
