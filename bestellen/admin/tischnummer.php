        ...
        <div class="u-clearfix u-expanded-width u-layout-wrap u-layout-wrap-1">
          ...
          <div class="apple_settings">
            <p style="padding: 10px 10px 10px 10px"><strong>Neuen Tisch hinzufügen:</strong>
              <form action="tischnummer_change.php?action=new" method="post" name="form" style="padding: 10px 10px 10px 10px">
            ...
            while ($zeile = mysqli_fetch_array( $sql_result, MYSQLI_ASSOC)) {
                if ($zeile['status'] == 'on') {
                    ...
                    echo '<div class="apple_'.$zeile['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>Tisch '.$zeile['nummer'].'</strong><p style="padding: 0px 0px 0px 10px"><a href="tischnummer_change.php?name='.$zeile['nummer'].'&status='.$zeile['status'].'&action=ch" class="'.$zeile['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br><br>';
                } else if ($zeile['status'] == 'off') {
                    $text = "Tisch aktivieren";
                    echo '<div class="apple_'.$zeile['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>Tisch '.$zeile['nummer'].'</strong><p style="padding: 0px 0px 0px 10px"><a href="tischnummer_change.php?name='.$zeile['nummer'].'&status='.$zeile['status'].'&action=ch" class="'.$zeile['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p><p style="padding: 0px 0px 0px 10px"><a href="tischnummer_change.php?name='.$zeile['nummer'].'&action=del" class="settings" style="color: black; padding: 5px 5px 5px 5px">Löschen</a></p></p></p></div><br><br>';
                }
            }
            ?>
          </div>
