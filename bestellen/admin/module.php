...
if (($name != "") & ($status != "")) {
  ...
  $sql_change = "SELECT * FROM `module` WHERE `abhaengig` = \"$name\"";
  $sql_change_result = mysqli_query($link, $sql_change);
  foreach ($sql_change_result as $row) {
    if ($row['status'] == "on") {
      header('location: module.php?error='.$row['name']);
      return;
    }
    $name_n = $row['name'];
    $sql_update1 = "UPDATE `module` SET `status` = \"$new_s\" WHERE `name` = \"$name_n\"";
    $sql_update1_result = mysqli_query($link, $sql_update1);
  }
  $sql_update2 = "UPDATE `module` SET `status` = \"$new\" WHERE `name` = \"$name\"";
  $sql_update2_result = mysqli_query($link, $sql_update2);  // Corrected line
    ...
    <section class="u-clearfix u-section-1" id="carousel_9c88">
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          ...
          $sql_modules_result = mysqli_query($link, query: $sql_modules);
          foreach ($sql_modules_result as $row) {
            if ($row['status'] != "error") {
                if ($row['status'] == 'on') {
                  $text = "✅ - Modul deaktivieren";
                  if ($row['settings'] == "") {
                    echo '<div class="apple_'.$row['status'].'"><p style="padding: 0px 10px 10px 10px"><strong>'.$row['name'].'</strong><br>'.$row['information'].'<p style="padding: 0px 0px 0px 10px"><a href="module.php?name='.$row['name'].'&status='.$row['status'].'" class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br>';
                  } else {
                    echo '<div class="apple_'.$row['status'].'"><p style="padding: 0px 10px 10px 10px"><strong>'.$row['name'].'</strong><br>'.$row['information'].'<p style="padding: 0px 0px 0px 10px"><a href="module.php?name='.$row['name'].'&status='.$row['status'].'" class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p><p style="padding: 0px 0px 0px 10px"><a href="'.$row['settings'].'" class="settings" style="color: black; padding: 5px 5px 5px 5px">⚙️ - Settings</a></p></p></div><br>';
                  }
                } else if ($row['status'] == 'off') {
                  $text = "❌ - Modul aktivieren";
                  echo '<div class="apple_'.$row['status'].'"><p style="padding: 0px 10px 10px 10px"><strong>'.$row['name'].'</strong><br>'.$row['information'].'<p style="padding: 0px 0px 0px 10px"><a href="module.php?name='.$row['name'].'&status='.$row['status'].'" class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br>';
                }
            } else {
              $text = "Gesperrt";
              echo '<div class="apple_'.$row['status'].'" style="padding: 0px 10px 10px 10px"><p><strong>'.$row['name'].'</strong><br>'.$row['information'].'
              <div class="apple_'.$row['status'].'" style="padding: 0px 10px 10px 10px">
              <h4>Modul kann nicht aktiviert werden!</h4>
              <p>'.$row['error'].'</p>
              </div>
              <p><a disabled class="'.$row['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br><br>';
            }
          }
          ?>
        </div>
