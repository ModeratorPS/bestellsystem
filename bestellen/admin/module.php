            ...
            while ($zeile = mysqli_fetch_array( $sql_result, MYSQLI_ASSOC)) {
                if ($zeile['status'] != "error") {
                    if ($zeile['status'] == 'on') {
                        ...
                        if ($zeile['settings'] == "") {
                            echo '<div class="apple_'.$zeile['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>'.$zeile['name'].'</strong><br>'.$zeile['information'].'<p style="padding: 0px 0px 0px 10px"><a href="module_change.php?name='.$zeile['name'].'&status='.$zeile['status'].'" class="'.$zeile['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br><br>';
                        } else {
                            echo '<div class="apple_'.$zeile['status'].'"><p style="padding: 10px 10px 10px 10px"><strong>'.$zeile['name'].'</strong><br>'.$zeile['information'].'<p style="padding: 0px 0px 0px 10px"><a href="module_change.php?name='.$zeile['name'].'&status='.$zeile['status'].'" class="'.$zeile['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p><p style="padding: 0px 0px 0px 10px"><a href="'.$zeile['settings'].'" class="settings" style="color: black; padding: 5px 5px 5px 5px">Settings</a></p></p></div><br><br>';
                        }
                    ...
                    $text = "Gesperrt";
                    echo '<div class="apple_'.$zeile['status'].'" style="padding: 10px 10px 10px 10px"><p><strong>'.$zeile['name'].'</strong><br>'.$zeile['information'].'
                    <div class="apple_'.$zeile['status'].'" style="padding: 10px 10px 10px 10px">
                        <h4>Modul kann nicht aktiviert werden!</h4>
                        <p>'.$zeile['error'].'</p>
                    </div>
                    <p><a disabled class="'.$zeile['status'].'" style="color: black; padding: 5px 5px 5px 5px">'.$text.'</a></p></p></div><br><br>';
                }
            }
            ?>
          </div>
