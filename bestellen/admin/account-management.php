...
if ($_POST["input1"] != "") {
  ...
  $sql_users_filtered = "SELECT * FROM `users` WHERE `username` = \"$input1\"";
  $sql_users_filtered_result = mysqli_query( $link, $sql_users_filtered );

  foreach ($sql_users_filtered_result as $row) {
    $sql_user_update = "UPDATE `users` SET `bewerten_rang`=\"$input2\" WHERE `username` = \"$input1\"";
    $sql_user_update_result = mysqli_query( $link, $sql_user_update );
  }
    ...
    <section class="u-clearfix u-section-1" id="carousel_9c88">
      <div class="infinite u-container-align-center u-container-style u-custom-color-2 u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-valign-middle u-container-layout-1">
          <form action="account-management.php" method="post" name="form">
            <select id="input1" name="input1">
              <?php
              $sql_users = "SELECT * FROM `users`";
              $sql_users_result = mysqli_query($link, $sql_users); 
              foreach ($sql_users_result as $row) {
            ...
            <input class="u-border-2 u-border-black u-btn u-button-style u-hover-black u-none u-text-hover-white u-btn-1" type="submit" name="submit" id="submit">
          </form>
        </div>
      </div>
    </section>
