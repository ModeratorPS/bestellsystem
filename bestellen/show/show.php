...
while ($zeileA = mysqli_fetch_array( $db_erg1, MYSQLI_ASSOC)) {
    $best = $zeileA['Bestellung'];
    $sql2 = "SELECT * FROM `Artikelliste` WHERE `artikel` = \"$best\"";
    $db_erg2 = mysqli_query( $link, $sql2 );
    while ($zeileB = mysqli_fetch_array( $db_erg2, MYSQLI_ASSOC)) {
        $time = $time + $zeileB['time'];
    }
...
if ($time <= $SHOW_lightgreen) {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/green.png" width="85" hight="85"><h1 style="color: green">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
} else if ($time <= $SHOW_darkgreen) {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/green.png" width="85" hight="85"><h1 style="color: olivedrab">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
} else if ($time <= $SHOW_orange) {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/gelb.png" width="85" hight="85"><h1 style="color: orange">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
} else if ($time <= $SHOW_darkorange) {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/gelb.png" width="85" hight="85"><h1 style="color: orangered">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
} else if ($time <= $SHOW_red) {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/rot.png" width="85" hight="85"><h1 style="color: red">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
} else {
    echo '<img style="float: left; margin-right: 10px;" src="wartezeit/rot.png" width="85" hight="85"><h1 style="color: darkred">Geschätzte Wartezeit: <strong>'.$time.' minuten</strong></h1>';
}
...
$sql4 = "SELECT * FROM `bestellungen` WHERE `Status` < 3";
